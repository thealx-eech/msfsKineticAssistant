        ...
        */
            ...
            if (validConnection())
            {
                ...
                if (towScanMode > TowScanMode.Disabled /*&& _nearbyInfoResponse.Count > 0*/)
                {
                    ...
                    try
                    {
                        ...
                        if (towingTarget == TARGETMAX && (_planeInfoResponse.AbsoluteTime - insertTowPlanePressed) > 6 && towScanMode >= TowScanMode.TowSearch && insertedTowPlane.Key != TARGETMAX && !insertedTowPlane.Value)
                        {
                        ...
                        if (lastPacketReceived != 0 /*1000 * lastFrameTiming > interval * 0.75*/ && lastFrameTiming > 0.001 && lastFrameTiming <= 1.0 && _planeAvionicsResponse.IsSlewActive != 100)
                        {
                            ...
                            if (_trackingClass.trackRecording != null && _trackingClass.recordingCounter > 0)
                            {
                                ...
                                else
                                {
                                    ...
                                    _trackingClass.lastRecordingCounter = _trackingClass.recordingCounter;
        ...
        private void trackControlChanges()
        {
            if (_planeAvionicsResponse.TotalWeight != 0)
            {
                ...
                if (_planeAvionicsResponse.WaterRudderHandlePosition >= 0 && _planeAvionicsResponse.WaterRudderHandlePosition != _planeAvionicsResponseLast.WaterRudderHandlePosition)
                {
        ...
        public void setTrackRecording(bool enable, bool save = true)
        {
            if (validConnection())
            {
                ...
                else
                {
                    ...
                    _trackingClass.lastRecordingCounter = 0;
